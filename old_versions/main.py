  ...
  <div class="two-col">
    ...
    <div class="main-panel">
      ...
      <div class="deploy">
        ...
        <ol>
          <li><span class="step">1</span>Run: <code>python main.py</code> — server on port 8000</li>
          <li><span class="step">2</span>Find your PC's local IP: open CMD and run <code>ipconfig</code></li>
          <li><span class="step">3</span>On each TV browser open: <code>http://YOUR_IP:8000/display</code></li>
          <li><span class="step">4</span>Each TV auto-registers in the Screens tab — assign playlists there</li>
          <li><span class="step">5</span>Enable kiosk mode in TV browser settings to hide browser UI</li>
        </ol>
      </div>
    </div>
  </div>
...
<div id="pw-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:none;align-items:center;justify-content:center">
...
let pwModalAdminId = null; // null = changing own password
...
async function submitPasswordChange(){
  ...
  if(pwModalAdminId === null){
    ...
    r = await authFetch('/api/auth/password',{method:'PUT',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({current_password:current, new_password:newPw})});
  } else {
    r = await authFetch(`/api/admins/${pwModalAdminId}/password`,{method:'PUT',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({new_password:newPw})});
  }
  if(r.ok){
    hidePasswordModal();
    toast(pwModalAdminId ? 'Password reset successfully' : 'Password changed successfully','success');
  } else {
    const d = await r.json().catch(()=>({}));
    errEl.textContent = d.detail || 'Error changing password';
    errEl.style.display='block';
  }
}
function openDisplay(){
  // Open real display (not preview) so orientation works correctly
  window.open('/display', '_blank');
}
function openPreview(){
  const url = activePid ? `/display?preview=${activePid}` : '/display';
...
function connectWS(){
  ...
  // Clear YouTube error timer when player signals it's ready
  window.addEventListener('message', e => {
    if(e.origin && e.origin.includes('youtube')) {
      const frame = document.getElementById('yt-frame');
      if(frame && frame._errTimer){
        clearTimeout(frame._errTimer);
        dbg('YouTube player ready');
      }
    }
  });
  ws.onmessage=e=>{
...
function renderPlChips(){
  ...
  document.getElementById('pl-chips').innerHTML=playlists.map(p=>`
    <div style="display:inline-flex;align-items:center;gap:0;margin-bottom:4px">
      <button class="pl-chip ${p.id===activePid?'active':''}" style="border-radius:20px 0 0 20px;border-right:none" onclick="selectPlaylist(${p.id})">${p.name}${p.fallback_url?'<span style="color:#fbbf24;margin-left:4px" title="YouTube fallback enabled">⚡</span>':''} <span style="opacity:.5;font-size:9px">${p.slide_count}</span></button>
      <button onclick="deletePlaylist(${p.id},'${p.name}')" title="Delete playlist" style="padding:6px 8px;border-radius:0 20px 20px 0;border:1px solid var(--border2);border-left:none;background:none;color:var(--muted);cursor:pointer;font-size:11px;line-height:1" onmouseenter="this.style.color='var(--red)'" onmouseleave="this.style.color='var(--muted)'">✕</button>
    </div>`).join('');
}
...
function thumb(s){
  if(s.type==='youtube') return null; // use placeholder instead of unreachable YouTube CDN
...
function toggleInterruptFields(enabled){
  document.getElementById('interrupt-fields').style.display = enabled ? 'block' : 'none';
}
function getInterruptEvery(){
  const h=parseInt(document.getElementById('f-int-h').value)||0;
  const m=parseInt(document.getElementById('f-int-m').value)||0;
  return h*3600+m*60;
}
function getInterruptFor(){
  const m=parseInt(document.getElementById('f-int-for-m').value)||0;
  const s=parseInt(document.getElementById('f-int-for-s').value)||0;
  return m*60+s;
}
...
async function saveFallback(){
  ...
  await authFetch(`/api/playlists/${activePid}/fallback`,{method:'PUT',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({fallback_url:url,loop_duration,fallback_duration})});
  toast('Fallback saved — TVs updated','success');
}
async function clearFallback(){
  if(!activePid)return;
  await authFetch(`/api/playlists/${activePid}/fallback`,{method:'PUT',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({fallback_url:null,loop_duration:1800,fallback_duration:300})});
  document.getElementById('fb-url').value='';
  toast('Fallback removed','success');
}
function updateDurPreview(){
  const tot=getDurSecs();
  document.getElementById('dur-preview').textContent='= '+fmtDur(tot);
}
async function addSlide(){
  ...
  const body={type:formType,url,title:document.getElementById('f-title').value.trim(),
    duration:getDurSecs(),
    sched_start:document.getElementById('f-sched-start').value||null,
    sched_end:document.getElementById('f-sched-end').value||null,
...
function setType(t){
  ...
  if(t==='youtube'){
    const cb=document.getElementById('interrupt-enabled');
    if(cb){cb.checked=false;toggleInterruptFields(false);}
  }
}
    ...
    <div class="screen-item" style="flex-wrap:wrap;gap:8px">
      ...
      <div class="screen-actions" style="flex-wrap:wrap">
        ...
        <div style="display:flex;gap:4px;align-items:center">
          ...
          <button onclick="setOrientation('${s.id}','landscape',90)"
            style="padding:4px 8px;border-radius:5px;border:1px solid var(--border2);background:${s.orientation==='landscape'?'var(--accent-dim)':'none'};color:${s.orientation==='landscape'?'var(--accent)':'var(--muted)'};font-size:10px;cursor:pointer">
            ▬ Land
          </button>
          <button onclick="pickPortrait('${s.id}')"
            style="padding:4px 8px;border-radius:5px;border:1px solid var(--border2);background:${s.orientation==='portrait'?'var(--accent-dim)':'none'};color:${s.orientation==='portrait'?'var(--accent)':'var(--muted)'};font-size:10px;cursor:pointer">
            ▮ Port ${s.orientation==='portrait'?(s.rotation_dir===90?'↻':'↺'):''}
          </button>
        </div>
...
async function setOrientation(sid, orientation, rotation_dir){
  await authFetch(`/api/screens/${sid}/orientation`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({orientation,rotation_dir})});
  toast(`Orientation updated`,'success');
}
function pickPortrait(sid){
  const dir = confirm('Rotate 90° clockwise?\nClick Cancel for counter-clockwise (90° CCW).') ? 90 : -90;
  setOrientation(sid, 'portrait', dir);
}
...
async function cleanupGhostScreens(){
  if(!confirm('Remove all screens that have been offline for 24+ hours?'))return;
...
async function loadAdmins(){
  ...
  admins=r.admins;
  // Load screen assignments for each subadmin
        ...
        <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px">
          ${screens.map(s=>`
            <label style="display:flex;align-items:center;gap:4px;font-size:11px;cursor:pointer">
              <input type="checkbox" class="screen-cb-${a.id}" value="${s.id}"
                ${(a.screen_ids||[]).includes(s.id)?'checked':''}/>
              <span style="color:${s.status==='online'?'var(--green)':'var(--muted)'}">${s.name}</span>
            </label>`).join('')}
...
#badge{position:fixed;top:14px;right:16px;font-size:10px;font-family:monospace;padding:4px 10px;border-radius:5px;z-index:9999;transition:opacity .5s}
...
#sname{position:fixed;top:14px;left:16px;font-size:10px;font-family:monospace;padding:4px 10px;border-radius:5px;background:rgba(0,0,0,.45);color:rgba(255,255,255,.4);z-index:9999}
</style>
</head>
<body>
<div id="wrapper">
  <div id="stage">
    <div id="yt-layer" class="layer hidden"><iframe id="yt-frame" allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen sandbox="allow-scripts allow-same-origin allow-presentation allow-popups"></iframe></div>
...
<div id="orient-btn" onclick="toggleOrientationLocal()" style="position:fixed;top:12px;right:12px;z-index:9999;background:rgba(0,0,0,.55);border:1px solid rgba(255,255,255,.2);border-radius:8px;padding:8px 12px;cursor:pointer;font-family:monospace;font-size:12px;color:rgba(255,255,255,.7);user-select:none">⟳</div>
<div id="badge" class="badge-bad">⬤ connecting...</div>
<div id="sname"></div>
<div id="debug" style="position:fixed;bottom:60px;left:10px;background:rgba(0,0,0,.8);color:#0f0;font-family:monospace;font-size:10px;padding:8px;border-radius:6px;z-index:9999;max-width:400px;display:none"></div>
<button onclick="document.getElementById('debug').style.display=document.getElementById('debug').style.display==='none'?'block':'none'" style="position:fixed;bottom:10px;left:10px;z-index:9999;padding:6px 10px;background:rgba(0,0,0,.6);color:#fff;border:1px solid #555;border-radius:5px;font-size:10px;cursor:pointer">Debug</button>
<script>
function dbg(msg){ const el=document.getElementById('debug'); el.innerHTML += msg+'<br>'; el.scrollTop=el.scrollHeight; }
</script>
<script>
function getScreenId(){
  // Priority 1: ?screen=name in URL (permanent, survives localStorage clear)
  ...
  // Priority 2: localStorage (persists across page refreshes in same browser)
...
snameEl.textContent = IS_PREVIEW ? `PREVIEW — playlist ${PREVIEW_PID}` : SCREEN_ID;
snameEl.style.color = IS_PREVIEW ? '#f59e0b' : 'rgba(255,255,255,.4)';
let playlist=[],idx=0,rafId=null,slideStart=0;
function tickClock(){const n=new Date();document.getElementById('clock-t').textContent=n.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});document.getElementById('clock-d').textContent=n.toLocaleDateString([],{weekday:'long',month:'long',day:'numeric'})}
setInterval(tickClock,1000);tickClock();
function ytId(url){const m=url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([^&\n?#]+)/);return m?m[1]:null}
function ytEmbed(url){
  const id=ytId(url);
  if(!id)return null;
  // Note: origin param removed — YouTube rejects non-HTTPS origins (http://192.168.x.x fails)
  return`https://www.youtube-nocookie.com/embed/${id}?autoplay=1&mute=1&rel=0&modestbranding=1&playsinline=1`;
}
function showSlide(i){
  ...
  if(s.type==='youtube'){
    ...
    if(src){
      ...
      if(frame._errTimer) clearTimeout(frame._errTimer);
      frame.src='about:blank';
      setTimeout(()=>{
        frame.src=src;
        document.getElementById('yt-layer').classList.remove('hidden');
        // If YouTube doesn't send a ready signal within 8s, skip to next slide
        frame._errTimer = setTimeout(()=>{
          const ytLayer = document.getElementById('yt-layer');
          if(!ytLayer.classList.contains('hidden')){
            dbg('YouTube timeout — skipping slide');
            ytLayer.classList.add('hidden');
  ...
  else if(s.type==='image'){document.getElementById('img-el').src=s.url;document.getElementById('img-layer').classList.remove('hidden')}
  else if(s.type==='video'){const v=document.getElementById('vid-el');v.src=s.url;v.play();document.getElementById('vid-layer').classList.remove('hidden')}
  ...
  document.getElementById('dots').innerHTML=playlist.map((_,di)=>`<div class="dot ${di===idx?'active':''}" style="${di===idx?'width:22px':'width:7px'}" onclick="jumpTo(${di})"></div>`).join('');
  cancelAnimationFrame(rafId);slideStart=performance.now();animPbar((s.duration||10)*1000);
}
function animPbar(durMs){
  const bar=document.getElementById('pbar');
  const tick=()=>{const e=performance.now()-slideStart;bar.style.width=Math.min((e/durMs)*100,100)+'%';if(e>=durMs){bar.style.width='0%';showSlide(idx+1)}else rafId=requestAnimationFrame(tick)};
  rafId=requestAnimationFrame(tick);
}
function jumpTo(i){cancelAnimationFrame(rafId);showSlide(i)}
// ── Interrupt engine ──────────────────────────────────────────────────────
// Each interrupt slide has its own timer. When it fires:
// 1. Pause normal playlist (cancel RAF, note current idx)
// 2. Show interrupt slide for interrupt_for seconds
// 3. Resume normal playlist from saved idx
let interruptTimers = [];
let savedIdx = 0;
...
function setupInterrupts(slides){
  // Clear old timers
  interruptTimers.forEach(t => clearInterval(t));
  interruptTimers = [];
  slides.forEach(s => {
    if(s.interrupt_every > 0){
      const t = setInterval(() => triggerInterrupt(s), s.interrupt_every * 1000);
      interruptTimers.push(t);
    }
  });
}

function triggerInterrupt(s){
  if(isInterrupting) return; // don't stack interrupts
  isInterrupting = true;
  savedIdx = idx; // remember where we were
  cancelAnimationFrame(rafId);
  document.getElementById('pbar').style.width = '0%';

  // Show the interrupt slide
  showInterruptSlide(s);

  // After interrupt_for seconds, restore normal playlist
  setTimeout(() => {
    isInterrupting = false;
    // restore saved position — cycle to next after the interrupt
    showSlide(savedIdx);
  }, s.interrupt_for * 1000);
}

function showInterruptSlide(s){
  ['yt','img','vid'].forEach(k=>document.getElementById(k+'-layer').classList.add('hidden'));
  if(s.type==='youtube'){
    const src=ytEmbed(s.url);
    if(src){document.getElementById('yt-frame').src=src;document.getElementById('yt-layer').classList.remove('hidden')}
  ...
  let elapsed=0;
  const durMs=s.interrupt_for*1000;
  const bar=document.getElementById('pbar');
  const countTick=()=>{
    if(!isInterrupting)return;
    elapsed+=100;
    bar.style.width=Math.min((elapsed/durMs)*100,100)+'%';
    if(elapsed<durMs)setTimeout(countTick,100);
  };
  setTimeout(countTick,100);
}

// ── Fallback engine ────────────────────────────────────────────────────────
// Tracks time spent in normal playlist. After loop_duration seconds,
// switches to YouTube fallback for fallback_duration seconds, then back.
let fallbackTimer = null;
let fallbackReturnTimer = null;
...
function setupFallback(url, loopSecs, fallbackSecs){
  ...
  clearTimeout(fallbackReturnTimer);
  currentFallbackUrl = url;

  if(!url || loopSecs < 1) return; // no fallback configured

  // Schedule first fallback after loopSecs
  fallbackTimer = setTimeout(() => triggerFallback(url, loopSecs, fallbackSecs), loopSecs * 1000);

  // Show a small countdown indicator
  showFallbackIndicator(loopSecs);
}

function triggerFallback(url, loopSecs, fallbackSecs){
  if(isInterrupting) {
    // Don't interrupt an interrupt — try again in 30s
    fallbackTimer = setTimeout(() => triggerFallback(url, loopSecs, fallbackSecs), 30000);
    return;
  }
  isFallback = true;
  savedIdx = idx;
  ...
  // Animate progress bar for fallback duration
  let elapsed=0;
  const bar=document.getElementById('pbar');
  const tick=()=>{
    if(!isFallback)return;
    elapsed+=200;
    bar.style.width=Math.min((elapsed/(fallbackSecs*1000))*100,100)+'%';
    if(elapsed < fallbackSecs*1000) setTimeout(tick,200);
  };
  setTimeout(tick,200);

  // Schedule return to normal playlist
  fallbackReturnTimer = setTimeout(() => {
    isFallback = false;
    bar.style.width='0%';
    showSlide(savedIdx);
    // Schedule next fallback cycle
    fallbackTimer = setTimeout(() => triggerFallback(url, loopSecs, fallbackSecs), loopSecs * 1000);
    showFallbackIndicator(loopSecs);
  }, fallbackSecs * 1000);
}
...
function showFallbackIndicator(loopSecs){
  // Small pill in corner showing when YouTube will next play
  const badge = document.getElementById('badge');
  let remaining = loopSecs;
  const tick = () => {
    if(isFallback || !currentFallbackUrl) return;
    remaining--;
    if(remaining <= 0) return;
    // Only show when < 2 mins remaining
    if(remaining <= 120){
      badge.textContent = `⚡ YouTube in ${fmtDur(remaining)}`;
...
function applyOrientation(orientation, rotDir){
  dbg('applyOrientation called: '+orientation+' rot='+rotDir);
  const w = document.getElementById('wrapper');
  if(!w){ dbg('ERROR: wrapper not found!'); return; }
  if(orientation === 'portrait'){
    const deg = rotDir || 90;
    // Set layout and transform separately — cssText with transform 
    // creates a stacking context that breaks YouTube iframe security checks
    ...
    // Reset to original CSS — critically: do NOT set transform at all.
    // Explicitly setting transform:none creates a stacking context that
    // breaks YouTube iframe security checks.
    w.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;overflow:hidden;';
    // Reload YouTube iframe — dimension change during portrait caused error state
    const frame = document.getElementById('yt-frame');
    if(frame && frame.src && frame.src.includes('youtube')) {
      const src = frame.src;
      frame.src = 'about:blank';
...
// ── Local orientation toggle ─────────────────────────────────────────────
let currentOrientation = 'landscape';
let currentRotDir = 90;

function toggleOrientationLocal(){
  if(currentOrientation === 'landscape'){
    // Ask clockwise or counter-clockwise via quick visual indicator
    currentOrientation = 'portrait';
    currentRotDir = currentRotDir === 90 ? 90 : -90;
    showOsdMsg('Portrait ▮');
  } else if(currentOrientation === 'portrait' && currentRotDir === 90){
    currentRotDir = -90;
    showOsdMsg('Portrait ↺');
  } else {
    currentOrientation = 'landscape';
    currentRotDir = 90;
    showOsdMsg('Landscape ▬');
  }
  applyOrientation(currentOrientation, currentRotDir);
  // Sync back to server so admin sees the change
  if(!IS_PREVIEW){
    fetch('/api/screens/' + SCREEN_ID + '/orientation', {
      method: 'PUT',
...
// Update currentOrientation when server pushes a change
function syncOrientationState(orientation, rotDir){
  currentOrientation = orientation || 'landscape';
  currentRotDir = rotDir || 90;
}
...
function connect(){
  ...
  ws.onmessage=e=>{
    ...
    if(msg.event==='playlist_updated'){
      dbg('SCREEN_ID='+SCREEN_ID+' orientation='+msg.orientation+' rot='+msg.rotation_dir);
      ...
      // Apply saved orientation immediately on connect
      if(msg.orientation){
        syncOrientationState(msg.orientation, msg.rotation_dir||90);
        // Only call applyOrientation if portrait — landscape is the default
        // and touching the wrapper CSS can break YouTube iframe embedding
        if(msg.orientation === 'portrait'){
          applyOrientation(msg.orientation, msg.rotation_dir||90);
        }
      }
    ...
    if(msg.event==='orientation_updated'){
      dbg('orientation_updated: '+msg.orientation+' rot='+msg.rotation_dir);
      syncOrientationState(msg.orientation, msg.rotation_dir);
      applyOrientation(msg.orientation, msg.rotation_dir);
    }
  };
  ws.onclose=()=>{const b=document.getElementById('badge');b.textContent='⬤ reconnecting...';b.className='badge-bad';b.style.opacity='1';setTimeout(connect,3000)};
...
# ── Bootstrap: create all required folders and write HTML files ───────────────
def bootstrap():
    for folder in ["templates", "static"]:
        Path(folder).mkdir(exist_ok=True)
    if not CLOUDINARY_CONFIGURED:
        Path("uploads").mkdir(exist_ok=True)
    # Always overwrite — never serve stale cached templates
    Path("templates/admin.html").write_text(ADMIN_HTML, encoding="utf-8")
    Path("templates/display.html").write_text(DISPLAY_HTML, encoding="utf-8")
    Path("templates/login.html").write_text(LOGIN_HTML, encoding="utf-8")
    print("[SignageOS] Templates written OK")
...
class ConnManager:
    ...
    def disconnect_screen(self, sid):
        self.screens.pop(sid, None)
    ...
    async def broadcast_admins(self, payload):
        """Broadcast to all admins — for non-screen events (playlists, groups, etc.)"""
        dead = []
    ...
    async def broadcast_screens_update(self):
        """Send filtered screens to each admin based on their role/assignments."""
        dead = []
...
async def push_playlist_to_screen(sid):
    ...
    # Get fallback config and orientation from DB
    conn = db._conn()
    try:
    ...
    await manager.push_to_screen(sid, {
        ...
        "loop_duration": fallback["loop_duration"],
        "fallback_duration": fallback["fallback_duration"],
        "orientation": screen["orientation"],
        "rotation_dir": screen["rotation_dir"]
    })
...
def get_screens_with_status():
    """Get screens with live is_connected flag included."""
...
def api_auth_test():
    return {"status": "auth endpoint reachable", "admins": len(db.get_all_admins())}
...
def api_get_admin_screens(request: Request, aid: int):
    require_superadmin(request)
    screen_ids = db.get_admin_screen_ids(aid)
    return {"screen_ids": screen_ids}
...
async def api_assign_screens(request: Request, aid: int, body: ScreenAssign):
    require_superadmin(request)
    # Replace all assignments for this admin
    conn = db._conn()
    try:
...
async def api_screen_orientation(request: Request, sid: str, body: OrientationUpdate):
    ...
    # Push orientation change to the specific screen via WebSocket
    await manager.push_to_screen(sid, {
        "event": "orientation_updated",
        "orientation": body.orientation,
        "rotation_dir": body.rotation_dir
    })
    await manager.broadcast_screens_update()
    return {"ok": True}
...
async def api_upload(file: UploadFile = File(...)):
    ...
    if CLOUDINARY_CONFIGURED:
        ...
        resource_type = "image" if slide_type == "image" else "video"
...
def api_get_screens(request: Request):
    ...
    for s in screens:
        s["is_connected"] = s["id"] in manager.online_ids
        # Real-time status — override DB value with live connection state
        s["status"] = "online" if s["is_connected"] else s.get("status","offline")
...
async def api_del_offline_screens(request: Request):
    """Delete all screens that have been offline for more than 24 hours."""
    require_superadmin(request)
    conn = db._conn()
    try:
        rows = db._execute(conn, """
...
async def ws_preview(ws: WebSocket, playlist_id: int):
    """Preview mode — serves a specific playlist without registering as a screen."""
    await ws.accept()
    try:
...
async def ws_admin(ws: WebSocket, token: str = ""):
    # Verify token passed as query param ?token=...
...
async def portrait_test(request: Request):
    from pathlib import Path
    html = Path("portrait_test.html").read_text(encoding="utf-8")
    from fastapi.responses import HTMLResponse
    return HTMLResponse(content=html, headers={"Cache-Control": "no-cache"})


@app.get("/yt-debug", response_class=HTMLResponse)
async def yt_debug_page(request: Request):
    from fastapi.responses import HTMLResponse
    html = Path("yt_test.html").read_text(encoding="utf-8")
...
async def admin_page(request: Request):
    # Let JS handle auth redirect — serve the page and JS checks the token
    return templates.TemplateResponse("admin.html", {"request": request})

@app.get("/display", response_class=HTMLResponse)
async def display_page(request: Request):
    from fastapi.responses import HTMLResponse as HR
    resp = templates.TemplateResponse("display.html", {"request": request})
