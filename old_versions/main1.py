...
#splash{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.93);display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99998;cursor:pointer}
</style>
</head>
<body>
  ...
  <div id="stage">
    <div id="yt-layer" class="layer hidden">
      ...
      <div id="yt-click-overlay">
        ...
        <div id="yt-newtab-btn" style="background:rgba(255,255,255,.15);color:#fff;padding:10px 22px;border-radius:8px;font-size:13px;font-family:sans-serif;border:1px solid rgba(255,255,255,.25)">
          ↗ Open in new tab
        </div>
      </div>
    </div>
...
<div id="orient-btn" onclick="toggleOrientationLocal()"
  style="position:fixed;top:12px;right:12px;z-index:9999;background:rgba(0,0,0,.55);border:1px solid rgba(255,255,255,.2);border-radius:8px;padding:8px 12px;cursor:pointer;font-family:monospace;font-size:12px;color:rgba(255,255,255,.7);user-select:none">⟳</div>
...
<div id="debug" style="position:fixed;bottom:60px;left:10px;background:rgba(0,0,0,.8);color:#0f0;font-family:monospace;font-size:10px;padding:8px;border-radius:6px;z-index:9999;max-width:400px;display:none;word-break:break-all"></div>
<button onclick="document.getElementById('debug').style.display=document.getElementById('debug').style.display==='none'?'block':'none'"
  style="position:fixed;bottom:10px;left:10px;z-index:9999;padding:6px 10px;background:rgba(0,0,0,.6);color:#fff;border:1px solid #555;border-radius:5px;font-size:10px;cursor:pointer">Debug</button>

<script>
// Crash Catcher: If you copy/pasted badly, this prints it to the screen instantly
...
function dismissSplash(){
  ...
  dbg('Splash dismissed — autoplay unlocked');
  if(pendingYTSlide){
...
function showYTClickOverlay(s){
  ...
  const newtabEl=document.getElementById('yt-newtab-btn');
  titleEl.textContent=s.title||'';
  overlay.style.display='flex';

  // Store window so we can close it
  newtabEl.onclick=function(e){
    e.stopPropagation();
    dbg('Opening YouTube in new tab');
    if(ytWindow&&!ytWindow.closed)ytWindow.close();
    ytWindow=window.open(s.url,'_blank');
  };

  overlay.onclick=function(e){
    if(e.target===newtabEl||newtabEl.contains(e.target))return;
    overlay.style.display='none';
    const frame=document.getElementById('yt-frame');
    frame.src=ytEmbed(s.url);
    dbg('Overlay tapped — reloading iframe synchronously');
  };
}
...
function showSlide(i){
  ...
  // Auto-close YouTube tab if it was opened
  if(ytWindow&&!ytWindow.closed){
    ytWindow.close();
    ytWindow=null;
    dbg('Closed YT window on slide advance');
  }
  ...
  animPbar((s.duration||10)*1000);
}

// Background-safe playlist timer
function animPbar(durMs){
  const bar=document.getElementById('pbar');
  clearTimeout(rafId);
  const tick=()=>{
    const e=performance.now()-slideStart;
    bar.style.width=Math.min((e/durMs)*100,100)+'%';
    if(e>=durMs){bar.style.width='0%';showSlide(idx+1);}
    else rafId=setTimeout(tick,250);
  };
  rafId=setTimeout(tick,250);
}

function jumpTo(i){clearTimeout(rafId);showSlide(i);}
...
function triggerInterrupt(s){
  ...
  isInterrupting=true;savedIdx=idx;
  clearTimeout(rafId);
  document.getElementById('pbar').style.width='0%';
  showInterruptSlide(s);
  setTimeout(()=>{isInterrupting=false;showSlide(savedIdx);},s.interrupt_for*1000);
}
...
function showInterruptSlide(s){
  ...
  // Background-safe real-time timer
  ...
  const countTick=()=>{
    ...
    if(elapsed<durMs)setTimeout(countTick,250);
  };
  setTimeout(countTick,250);
}

let fallbackTimer=null,fallbackReturnTimer=null,isFallback=false,currentFallbackUrl=null;
function setupFallback(url,loopSecs,fallbackSecs){
  clearTimeout(fallbackTimer);clearTimeout(fallbackReturnTimer);
...
function triggerFallback(url,loopSecs,fallbackSecs){
  ...
  const tick=()=>{
    ...
    const elapsed=performance.now()-startTime;
    bar.style.width=Math.min((elapsed/durMs)*100,100)+'%';
    if(elapsed<durMs)setTimeout(tick,250);
  };
  setTimeout(tick,250);

  fallbackReturnTimer=setTimeout(()=>{
    isFallback=false;bar.style.width='0%';showSlide(savedIdx);
    fallbackTimer=setTimeout(()=>triggerFallback(url,loopSecs,fallbackSecs),loopSecs*1000);
    showFallbackIndicator(loopSecs);
  },fallbackSecs*1000);
}
