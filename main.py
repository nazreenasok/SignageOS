  ...
  <div class="two-col">
    <div class="sidebar" style="padding:14px">
      ...
      <div class="card" style="margin-top:4px">
        ...
        <div class="type-grid">
          <button class="type-btn act-image" onclick="setType('image')">◼ Image</button>
          <button class="type-btn" onclick="setType('video')">⬤ Video</button>
          <button class="type-btn" onclick="setType('upload')">⬆ Upload</button>
        </div>
        <div id="url-section">
          <div class="form-group">
            <label id="url-label">Image URL</label>
            <input class="inp" id="f-url" placeholder="https://example.com/image.jpg"/>
          </div>
        </div>
    ...
    <div class="main-panel">
      ...
      <div class="deploy">
        ...
        <ol>
          ...
          <li><span class="step">4</span>Each TV auto-registers in the Screens tab</li>
        </ol>
      </div>
    </div>
  </div>
...
function renderPlChips(){
  ...
  document.getElementById('pl-chips').innerHTML=playlists.map(p=>`
    <div style="display:inline-flex;align-items:center;gap:0;margin-bottom:4px">
      <button class="pl-chip ${p.id===activePid?'active':''}" style="border-radius:20px 0 0 20px;border-right:none" onclick="selectPlaylist(${p.id})">${p.name} <span style="opacity:.5;font-size:9px">${p.slide_count}</span></button>
...
async function deletePlaylist(pid,name){
  ...
  if(activePid===pid){activePid=null;document.getElementById('slides-list').innerHTML='';document.getElementById('now-bar').style.display='none';}
  await loadPlaylists();
...
function renderSlides(){
  ...
  el.innerHTML=slides.map((s,i)=>{
    const t=thumb(s);
    const thumbHtml=t?`<img src="${t}" alt="" onerror="this.style.display='none'">`:`<div style="font-size:16px;opacity:.3">${s.type==='video'?'🎬':'◼'}</div>`;
...
function setType(t){
  ...
  ['image','video','upload'].forEach((x,i)=>{if(x===t)document.querySelectorAll('.type-btn')[i].className=`type-btn act-${t}`;});
  const ph={image:'https://example.com/image.jpg',video:'https://example.com/video.mp4'};
  document.getElementById('url-label').textContent={image:'Image URL',video:'Video URL'}[t]||'URL';
...
let playlist=[],idx=0,timerId=null,slideStart=0;
...
function showSlide(i){
  ...
  // Clean up any remaining YouTube slides in the db by skipping them
  if(s.type === 'youtube') {
      dbg('Skipping legacy YouTube slide');
      setTimeout(()=>showSlide(idx+1), 500);
      return;
  }
  ...
  const labels={image:'Poster',video:'Video'};
...
function animPbar(durMs){
  ...
  const tick=()=>{
    ...
    else timerId=setTimeout(tick,250);
  };
...
if __name__ == "__main__":
    ...
    uvicorn.run("no_yt:app", host="0.0.0.0", port=port, reload=False)